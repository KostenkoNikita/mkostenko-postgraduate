...
export function fullFirstKindEllipticIntegral(m: number, precision: number = EPS) {
   const m1 = 1.0 - m;
   ...
   if(m1 <= precision) {
      return 1.3862943611198906188E0-0.5 * Math.log(m1);
   } else {
      ...
      return p - q * Math.log(m1);
   }
}

export function fullSecondKindEllipticIntegral(m: number) {
   m = 1.0 - m;
